...
class HybridMessageAggregator(MessageAggregator):
    def __init__(self, device, embedding_dimension):
        super(HybridMessageAggregator, self).__init__(device)
    ...
    def aggregate(self, node_ids, messages):
        ...
        for node_id in unique_node_ids:
            if len(messages[node_id]) > 0:
                to_update_node_ids.append(node_id)
                temp_message2 = torch.cat([torch.mean(messages[node_id][0][:,:self.embedding_dimension],dim=0),
                                           torch.sum(messages[node_id][0][:,self.embedding_dimension:],dim=0)])
                unique_messages.append(temp_message2)
                unique_timestamps.append(messages[node_id][1][-1])
        unique_messages = torch.stack(unique_messages) if len(to_update_node_ids) > 0 else []
        unique_timestamps = torch.stack(unique_timestamps) if len(to_update_node_ids) > 0 else []
...
def get_message_aggregator(aggregator_type, device, embedding_dimension=0):
    ...
    elif aggregator_type == "hybrid":
        return HybridMessageAggregator(device=device, embedding_dimension=embedding_dimension)
    else:
